		...
		<form action="/updateMember" method="post" id="mypage">
			...
			<table class="table">
				...
				<tr>
					...
					<td>
						<input type="text" name="memberPw" id="memberPw" class="form-control" value="${m.memberPw }">
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th><label for = "enrollDate">가입일자</label></th>
					<td>
						<input type="text" name="enrollDate" id="enrollDate" class="form-control" value="${m.enrollDate }" readonly>
					</td>
				</tr>
			</table>
			<div id = "btn-box">
				...
				<c:if test="${m.memberId eq 'admin' }">
					<button class="btn btn-outline-danger btn-lg" type="button" onclick="location.href='/adminPage'">회원 관리</button>
				</c:if>
				<c:if test="${m.memberId ne 'admin' }">
					<button class="btn btn-outline-danger btn-lg" type="button" onclick="deleteMember('${m.memberId}')">회원 탈퇴</button>
				</c:if>
			</div>
		</form>
	</section>
